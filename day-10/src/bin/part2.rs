...
fn process(input: &str) -> String {
    ...
    let boundary_map = get_boundary_map(&map);
    ...
    let result = map
        ...
        .map(|line| {
            ...
            line.iter()
                .filter(|tile| {
                    if boundary_map[tile.row][tile.column] {
                        if [
                            Pipe::Start,
                            Pipe::Vertical,
                            Pipe::SouthWest,
...
fn get_boundary_map(map: &Vec<Vec<Tile>>) -> Vec<Vec<bool>> {
    let mut boundary_map: Vec<Vec<bool>> = vec![vec![false; map[0].len()]; map.len()];
    ...
    boundary_map[start.row][start.column] = true;

    let (mut path1, mut path1_dir, mut path2, mut path2_dir) =
        get_first_two_connecting_pipes(start, &map);

    loop {
        boundary_map[path1.row][path1.column] = true;
        boundary_map[path2.row][path2.column] = true;
        ...
        if path1_next == path2_next {
            boundary_map[path1_next.row][path1_next.column] = true;
            boundary_map[path2_next.row][path2_next.column] = true;
            return boundary_map;
        } else {
            path1 = path1_next;
            path2 = path2_next;
...
impl Tile {
    ...
    fn get_next(&self, direction: &Direction, map: &Vec<Vec<Tile>>) -> Option<Tile> {
        let other: &Tile = match direction {
            ...
            Direction::East => {
                if self.column + 1 >= map[self.row].len() {
                    return None;
                }
...
mod tests {
    ...
    #[test]
    fn it_works2_simple() {
        let input = include_str!("./input_test2_simple.txt");
        let result = process(input);
        assert_eq!(result, "4");
    }

    #[test]
    fn it_works2_complex() {
        let input = include_str!("./input_test2_complex.txt");
        let result = process(input);
        assert_eq!(result, "8");
    }
}
